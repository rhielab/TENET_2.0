...
cat hyper.G+.link.geneID.txt hyper.G-.link.geneID.txt > hyper.link.geneID.txt
sort hyper.link.geneID.txt | uniq > hyper.link.geneID.uniq.txt
Rscript ../../scripts/step3/TENET_unique_geneID_subsetting_script_hyper.R
...
Rscript ../../scripts/step3/get.permutation.z.score.for.hypermeth.enhancers.bringfiles.realfast.from.step2.R
allrda1=$(wc -l ./hyper.G+.output/*perm.txt)
allrda1c=$(echo $allrda1 | awk '{print $1}')
allrda2=$(wc -l ./hyper.G-.output/*perm.txt)
allrda2c=$(echo $allrda2 | awk '{print $1}')
if [[ $allrda1c > 0 && $allrda2c > 0 ]];
rm *fin.txt;
then echo "hyper permutaion is done";
fi
