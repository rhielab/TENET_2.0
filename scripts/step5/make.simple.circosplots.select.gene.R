...
load("../settings.rda")
GL=list.files("../external.data/otherinfo", "gene")
genelist=read.delim(paste("../external.data/otherinfo/",GL[1],sep=""),header=F)
links=paste(as.character(strsplit(GL[1], "\\.")[[1]][1]), as.character(strsplit(GL[1], "\\.")[[1]][2]), sep=".")
geneName=as.character(genelist$V1)
library(parallel)
library(ggplot2)
library(RCircos)
...
makeCircosG=function(geneName=c(), all,circos.col=c()){
allc=all[which(all$geneSymbol==geneName),]
...
Links=genes
Links$V1=probes$chr
Links$V2=probes$start
Links$V3=probes$end
Links$V4=probes$name
...
out.file <- paste(prefix, geneName, links, "links.circosplot.pdf", sep=".")
...
mclapply(geneName, makeCircosG, all=all, circos.col=circos.col, mc.cores=cores)
