...
## Than normal sample mean, and has less than the hypermeth cutoff:
LUAD_tumor_sample_link_evaluator <- function(methylation_probe_link, gene_link){
  ...
  ## For each methylation sample, check if it is hypermethylated for the given probe:
  cutoff_values <- ifelse(
    tumor_methylation_values_for_probe > hypercutoff,
    1,
    0
  )
...
## Calculate if hypermeth link is present:
...
write.table(
  ...
  file='./hyper.G+.output.states/links.states.table.tsv',
  quote= FALSE,
  sep='\t'
)