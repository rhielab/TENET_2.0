...
p=ggplot(DATA_i, aes(x=DATA_i[,2], y=DATA_i[,1]))+scale_shape_identity()+geom_point(colour=as.character(DATA_i$colour), size=as.numeric(Purity), shape=as.numeric(CNV.sh2))+ylab(paste(names(DATA_i)[1], "DNA methylation", sep=" "))+xlab(paste(TESTSR[i,2], "gene expression level", sep=" "))+ylim(0,1)+theme_bw()+ggtitle(paste("Complex Scatterplot for ", TESTSR[i,2], ":", TESTSR[i,1],sep=" "))
