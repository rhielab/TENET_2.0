...
get.zscore.from.pval <- function(Zcutoff) {
    load("../../settings.rda")
    pval=Zcutoff
    print(qnorm(1 - (pval)))
}

get.pairs.z.score.for.hypermeth.enhancers.realfast <- function(parallel, prefix, usecaseonly, expDataT, hypermethDataT, hypercutoff, expDataN, hypermethDataN, minTumor, i, cores) {
    # library(parallel)
    load("../settings.rda")
...
get.pairs.z.score.for.hypometh.enhancers.realfast <- function(parallel, prefix, usecaseonly, expDataT, hypomethDataT, hypocutoff, expDataN, hypomethDataN, minTumor, i, cores) {
    # library(parallel)
    ...
    # let's load diffmethylated enhancer region DNA meth data #
    load(paste("../step1/output/", prefix, ".diff.methylated.datasets.rda", sep=""))
    ## for hypomethylated probes ##
    ### let's make a table containing Z score ##
    if (usecaseonly==T){
        expData=expDataT
        metData=ifelse(hypomethDataT<hypocutoff,1,0)}
    if (usecaseonly==F){
        expData=cbind(expDataT, expDataN)
        metData=ifelse(cbind(hypomethDataT, hypomethDataN)<hypocutoff,1,0)}
