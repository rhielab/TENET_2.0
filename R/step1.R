get.diffmeth.regions <- function(settings,
                                 gencode_file = 'data/gencode_v22_gtf_file.tsv.gz',
                                 hg38_450k_probe_info_file = 'data/hm450.hg38.manifest.tsv.gz',
                                 udist = 1500) {
    ...
    gencode_v22_TSS <- bind_ranges(gencode_v22_genes, gencode_v22_transcript)
    ...
    gencode_v22_TSS <-
        gencode_v22_TSS %>%
        dplyr::mutate(TSS = as.character(strand(.))) %>%
        dplyr::mutate(TSS = dplyr::case_when(TSS == "+" ~ start,
                                             TSS == "-" ~ end)) %>%
        identity()
    ...
    gencode_v22_TSS_granges <-
        gencode_v22_TSS %>%
        as_tibble() %>%
        dplyr::mutate(start = dplyr::case_when(strand == "+" ~ TSS-udist,
                                               strand == "-" ~ TSS-1-udist)) %>%
        dplyr::mutate(end = dplyr::case_when(strand == "+" ~ TSS+1+(udist-2),
                                               strand == "-" ~ TSS+(udist-2))) %>%
        dplyr::mutate(width = end - start+1, strand = "*") %>%
        dplyr::mutate(name = paste(stringr::str_remove(gene_id, '\\..*'),
                                  type,
                                  sep='_')) %>%
        identity() %>%
        as_granges()
    ...
    hg38_450k_probe_info <- read.delim(
        file=hg38_450k_probe_info_file,
        header= TRUE,
        stringsAsFactors = FALSE
    )
    ...
    if (elmerENH==T){
        ...
        TENET_1.5.1_file <- read.delim(
            'data/ELMERv.1.5.1_TENET_probes.tsv',
            stringsAsFactors = FALSE
        )
    ...
    if (extENH==T){
        external.enhancer=read.delim("step1/hm450overlap/enhancer/external.data.enhancer.hm450.probelist.txt", header=F)
    }
    if (extENH==F){
    ...
    if (encodeNDR==T){
        ...
        NDR=read.delim("data/wgEncodeAwgDnaseMasterSites.hg38.hm450cg_GEO_GR.bed.probelist.txt", header=T)
    }
    if (encodeNDR==F){
        NDR=data.frame(V1=c(NA))
    }
    if (extNDR==T){
        external.NDR=read.delim("step1/hm450overlap/NDR/external.data.NDR.hm450.probelist.txt", header=F)
    }
    if (extNDR==F){
    ...
    if (onlyextFeature==T){
        external.Feature=read.delim("step1/hm450overlap/feature/external.data.feature.hm450.probelist.txt", header=F)
    ...
    annoGenes=read.delim("data/gene.anno.hg38.txpt.bed", header=T)
...
make.rdafiles.from.textfiles.for.othercells <- function(settings) {
    # load("../../settings.rda")
    ET=list.files("txt")
    annoHM450=read.delim("data/hm450cg_GEO.GR.bed", header=F)
